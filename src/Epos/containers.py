...
class Epo:
    PARSED_PATH = PathTemplate('$pdata/epo/epo_$index.pkl.gz')
    ...
    def load_selected(cls, index, load=True):
        ...
        res = {pub_number for pub_number, epo in chain.items() if cls._CPC_TREE_.any_selected(epo.cpcs)}
        if not load:
            del cls._CPC_TREE_
        return res
    ...
    def from_parse(cls,
        ...
        if pub_number:
            ...
            print("Neither publication nor application number provided. Initiated with Epo('_Anonymous_')'")
            new = cls('_Anonymous_')
