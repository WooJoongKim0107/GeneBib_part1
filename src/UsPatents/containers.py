...
from .cpc import CPCTree, IPCTree


class UsPatent:
    PARSED_PATH = PathTemplate('$pdata/us_patent/patent_$index.pkl.gz')
    ...
    def load_selected(cls, index, use_cpc: bool, load=True):
        ...
        res = {pub_number for pub_number, us_pat in chain.items() if cls._TREE_.any_selected(us_pat.cpcs)}
        if not load:
            del cls._TREE_
        return res
    ...
    def from_parse(cls,
        ...
        if pub_number:
            ...
            print("Neither publication nor application number provided. Initiated with UsPatent('_Anonymous_')'")
            new = cls('_Anonymous_')
    ...
    def __repr__(self):
        return f"UsPatent({self.pub_number})"
